...
describe("checklist application", () => {
  ...
  it("offers recovery actions when focus mode hides a completed category", async () => {
    ...
    expect(screen.getByRole("button", { name: "Раздел Таблицы" })).toHaveTextContent(
      "Готово",
    );
    await user.click(screen.getAllByRole("switch")[0]);
    expect(screen.getByText("Все релевантные пункты выполнены")).toBeInTheDocument();

    await user.click(screen.getByRole("button", { name: "Показать всё" }));
    expect(screen.queryByText("Все релевантные пункты выполнены")).not.toBeInTheDocument();
    await user.click(screen.getByRole("button", { name: "Снять отметки" }));
    expect(
