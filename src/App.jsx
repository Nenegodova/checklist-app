...
const DATA = {
  "Админка": [
    { text: "Проверить подзаголовок, слаг, редакции, теги, потоки, формат, вопрос к читателям, мету, краткий заг" },
    { text: "Проверить правильность формата" },
    { text: "Ог-заг = заголовок статьи, ог-описание на месте" },
    { text: "Проверить скрытие" },
    { text: "Проврить тип обложки, подпись к обложке в нужном месте (под обложкой/в подвале), актуальность текста, наличие бирки, текст на ОГ оттипирован (проставлены склейки),18+ не обрезается на ОГ)" },
  ...
  "Текст": [
    ...
    { text: "Внутри склеяны *<nobr></nobr>* диапазоны чисел, составные наречия, °C, слова, пишущиеся через дефис слова (до 4 символов от дефиса" },
    ...
    { text: "Cервисных плашках в последнем предложении отсутствует точка" },
    { text: "Нет пустых атрибутов" },
    { text: "UTM метки отсутствуют" },
    { text: "У первого валютного фичера тултип про пересчет" },
