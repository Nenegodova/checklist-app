...
export function getHiddenByFiltersCount(tasks, relevantTasks) {
  const total = Object.values(tasks).reduce((count, category) => count + category.length, 0);
  const relevant = Object.values(relevantTasks).reduce((count, category) => count + category.length, 0);
...
export function getVisibleTasks(relevantTasks, focusMode) {
  ...
  return Object.fromEntries(
    Object.entries(relevantTasks).map(([category, tasks]) => [category, tasks.filter((task) => !task.done)]),
  );
}
