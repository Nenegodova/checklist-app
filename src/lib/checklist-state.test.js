...
describe("checklist state", () => {
  it("keeps every task when all filters are enabled", () => {
    ...
    expect(getOverallProgress(relevant)).toEqual({ done: 3, total: 4, percent: 75 });
  });
  ...
  it("filters feature tasks and the complete Tables category without double counting", () => {
    ...
    expect(getCategoryProgress(relevant, "Таблицы")).toEqual({ done: 0, total: 0 });
    expect(getOverallProgress(relevant)).toEqual({ done: 0, total: 1, percent: 0 });
  });
  ...
  it("does not let focus alter progress, while hiding completed relevant tasks", () => {
    ...
    expect(getOverallProgress(relevant)).toEqual({ done: 1, total: 2, percent: 50 });
    expect(getVisibleTasks(relevant, true)).toEqual({
      "Текст": [{ id: "plain", done: false }],
      "Таблицы": [],
    });
    expect(getCategoryProgress(relevant, "Таблицы")).toEqual({ done: 1, total: 1 });
  });
});
